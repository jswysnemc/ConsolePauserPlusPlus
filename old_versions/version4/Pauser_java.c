...
int exec_c_cpp(int argc,char *argv[]){
    if (argc < 2 || argc > 4) 
        return printf("使用方法:\n\n <程序名称> <输入文件> <输出文件>\n"), -1;
...
int exec_java(int argc,char *argv[]){

     if (argc < 3 || argc > 6) 
        return printf("使用方法:\n\n Pauser <java> <类路径> <类名> <输入文件> <输出文件>\n"), -1;
    ...
    if(argc == 6){
        ...
        sprintf(CommandLine, "java -classpath %s  \"%s\"  < \"%s\" > \"%s\" ", argv[2], argv[3],argv[4],argv[5]);
        freopen(argv[4], "r", stdin);
        freopen(argv[5], "w", stdout);
    }

    if(argc == 4){
        sprintf(CommandLine, "java -classpath %s  \"%s\" ", argv[2], argv[3]);
    }
...
int main(int argc, char *argv[])
{
    ...
    if(strcmp(argv[1],"java")==0){
        return exec_java(argc, argv);
    }else{
        return exec_c_cpp(argc, argv);
    }
    return 0;
}